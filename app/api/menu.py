from flask import session, current_app, g, jsonify
from app.api import bp
from app.api.auth import token_auth
from app.utils.rights import permission_required
#from app.api.permission import permission_required
from app.models.models import Permission, AdminUser,AdminPermission
from app.utils import common
from app.utils.schema import AdminPermissionSchema

@bp.route('/menu/menu', methods=['GET'])
@token_auth.login_required
#@permission_required(Permission.READ)
def get_menu():
    """
    菜单数据
    """
    user = AdminUser.query.get(g.uid)
    role = user.role
    power0 = []
    power1 = []


    if user.id == 1:
        # 超级管理员直接获取全部
        p = AdminPermission.query.all()
        for i in p:
            if i.state == 0:
                continue
            if int(i.type) == 0:
                power0.append(i)
            else:
                power1.append(i)
    else:
        for i in role:
            # 如果角色没有被启用就直接跳过
            if i.state == 0:
                continue
            # 变量角色用户的权限
            for p in i.permission:
                # 如果权限关闭了就直接跳过
                if p.state == 0:
                    continue
                # 一级菜单
                if int(p.type) == 0:
                    power0.append(p)
                # 二级菜单
                else:
                    power1.append(p)
    power_schema = AdminPermissionSchema(many=True)  # 用已继承 Marshmallow.ModelSchema 类的自定制类生成序列化类
    power0_dict = power_schema.dump(power0)  # 生成可序列化对象
    power1_dict = power_schema.dump(power1)  # 生成可序列化对象
    power0_dict = sorted(power0_dict, key=lambda i: i['sort'])
    power1_dict = sorted(power1_dict, key=lambda i: i['sort'])
    # print(power0)
    # print(power1)

    menu = []

    for p0 in power0_dict:
        for p1 in power1_dict:
            if p0.get('id') == p1.get('parent_id'):
                if p0.get("children") is None:
                    p0['children'] = []
                p0['children'].append(p1)
        menu.append(p0)
    return jsonify(menu)

@bp.route('/menu/configs', methods=['GET'])
@token_auth.login_required
#@permission_required(Permission.READ)
def get_render_config():
    # 网站配置
    config = dict(logo={
        # 网站名称
        "title": current_app.config.get("SYSTEM_NAME"),
        # 网站图标
        "image": "/static/pear/admin/images/logo.png"
        # 菜单配置
    }, menu={
        # 菜单数据来源
        "data": "/api/menu/menu",
        # 侧边默认折叠状态
        "collaspe": False,
        # 是否同时只打开一个菜单目录
        "accordion": True,
        "method": "GET",
        # 是否开启多系统菜单模式
        "control": False,
        # 默认选中的菜单项
        "select": "0",
        # 是否开启异步菜单，false 时 data 属性设置为菜单数据，false 时为 json 文件或后端接口
        "async": True
    }, tab={
        # 是否开启多选项卡
        "muiltTab": True,
        # 切换选项卡时，是否刷新页面状态
        "keepState": True,
        # 是否开启 Tab 记忆
        "session": True,
        # 最大可打开的选项卡数量
        "tabMax": 30,
        "index": {
            # 标识 ID , 建议与菜单项中的 ID 一致
            "id": "10",
            # 页面地址
            "href": "/welcome",
            # 标题
            "title": "首页"
        }
    }, theme={
        # 默认主题色，对应 colors 配置中的 ID 标识
        "defaultColor": "2",
        # 默认的菜单主题 dark-theme 黑 / light-theme 白
        "defaultMenu": "dark-theme",
        # 是否允许用户切换主题，false 时关闭自定义主题面板
        "allowCustom": True
    }, colors=[{
        "id": "1",
        "color": "#2d8cf0"
    },
        {
            "id": "2",
            "color": "#5FB878"
        },
        {
            "id": "3",
            "color": "#1E9FFF"
        }, {
            "id": "4",
            "color": "#FFB800"
        }, {
            "id": "5",
            "color": "darkgray"
        }
    ], links=current_app.config.get("SYSTEM_PANEL_LINKS"), other={
        # 主页动画时长
        "keepLoad": 1200,
        # 布局顶部主题
        "autoHead": False
    }, header=False)
    return jsonify(config)
