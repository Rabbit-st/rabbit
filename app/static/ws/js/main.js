...
jQuery(function ($) {
    ...
    function ajax_complete_callback(resp) {
        ...
        sock.onmessage = function (msg) {
            // 接收后端的数据
            //read_file_as_text(new Blob([new Uint8Array(msg.data)]), term_write, decoder);
    ...
    function connect() {
        ...
        args = searchToObject();
        if (args.host_id === undefined) {
            console.log('host_id not fund');
            return false;
        }
        if (args.ip && args.id) {
            default_title = args.ip + '-' + args.id.toString();
            title_element.text = args.ip + '-' + args.id.toString();
        }
