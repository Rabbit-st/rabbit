...
from app.ws.worker import Worker, recycle_worker, clients
# mysql
from app.ws.db import DB
# 密码解密
from app.utils import pscrypt, Token, ssh_key_string_to_obj
...
class MixinHandler(object):
    ...
    def get_value(self, name):
        value = json_decode(self.request.body).get(name)  # json格式转为dict
...
class IndexHandler(MixinHandler, tornado.web.RequestHandler):
    ...
    def initialize(self, loop, policy, host_keys_settings):
        ...
        self.result = dict(id=None, status=None, encoding=None, ip=None)
        self.db = db
    ...
    def get_args(self, host_id):
        if not host_id:
            raise InvalidValueError('Invalid server id: {}'.format(host_id))
        cur = DB.get_cursor(self.db)
        try:
            cur.execute("SELECT h.host_ip, h.ssh_port, u.ssh_user, u.ssh_password, u.private_key "\
                        "FROM hosts h "\
                        "LEFT JOIN ssh_users u " \
                        "ON h.ssh_id = u.id " \
                        "WHERE h.id = %s", (host_id))
            data = cur.fetchone()
        except Exception as e:
            raise InvalidValueError('mysql connection failure.')

        if not data:
            raise InvalidValueError('hostid {} not exists.'.format(host_id))
        ...
        if not password and not privatekey:
            raise InvalidValueError('密码跟密钥都未配置')
            
        # 优先用密码登录
        if password:
            password = pscrypt.decrypt(password)
        elif privatekey:
            privatekey = pscrypt.decrypt(privatekey)

        if isinstance(self.policy, paramiko.RejectPolicy):
            self.lookup_hostname(hostname, port)

        if privatekey:
            pkey = ssh_key_string_to_obj(privatekey)
        else:
            pkey = None
    ...
    def ssh_connect(self, args):
        ...
        term = self.data.get('term', u'') or u'xterm'
    ...
    def check_origin(self):
        event_origin = self.data.get('_origin', u'')
    ...
    def get(self):
        self.render('templates/webssh.html')

    @tornado.gen.coroutine
    def post(self):
        self.data = json_decode(self.request.body)  # post 请求参数
        if 'Authorization' not in self.request.headers:
            raise tornado.web.HTTPError(403, 'Unauthorized access')
        else:
            auth_type, token = self.request.headers.get('Authorization').split(
                                    None, 1)

        c = Token.verify_auth_token(token)
        if not c:
            raise tornado.web.HTTPError(403, 'Unauthorized access')

        if self.debug and self.data.get('error', u''):
        ...
        #self.check_origin()
        try:
            args = self.get_args(self.data.get('host_id'))
        ...
        try:
            ...
            self.result.update(id=worker.id, encoding=worker.encoding, title=args[0])  # title设置为ip
